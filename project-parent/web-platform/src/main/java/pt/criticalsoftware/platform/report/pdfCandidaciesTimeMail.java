...
@Named
@RequestScoped
public class pdfCandidaciesTimeMail {
	private final Logger logger = LoggerFactory.getLogger(pdfCandidaciesTimeMail.class);
	...
	public String generatMain(ArrayList<ICandidacy> candidacies, String documentNumber){
		...
		String FILE = "candidaciesByPeriod"+documentNumber+".pdf";
		try {
			...
			PdfPCell cell2 = new PdfPCell(new Paragraph("Referencia da Posicao"));
			...
			for (ICandidacy interv:candidacies){
				...
				table.addCell(interv.getPositionCandidacy().getReference());
	...
	private void addTitlePage(Document document)
		...
		preface.add(new Paragraph("Relatório Candidaturas Por Período De Tempo", catFont));
