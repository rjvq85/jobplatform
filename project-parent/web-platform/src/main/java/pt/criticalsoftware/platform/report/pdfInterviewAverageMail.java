...
@Named
@RequestScoped
public class pdfInterviewAverageMail {
	...
	public String generatMain(ArrayList<ICandidacy> candidacies, String documentNumber){
		...
		String FILE = "candidaciesAverage"+documentNumber+".pdf";
		try {
			...
			PdfPCell cell4 = new PdfPCell(new Paragraph("Data da Primeira Entrevista"));
			...
			for (ICandidacy interv:candidacies){
				...
				table.addCell(interv.getInterviews().get(0).getDate().toString());
			}
			document.add(table);
			document.close();
		} catch (Exception e) {
	...
	private void addMetaData(Document document) {
		document.addTitle("Relatorio Primeira Media Entrevista");
	...
	private void addTitlePage(Document document)
		...
		preface.add(new Paragraph("Relatório Do Tempo Médio Para a Primeira Entrevista", catFont));
