...
@Named
@RequestScoped
public class pdfNonAdmitedMail {
	...
	public String generatMain(ArrayList<ICandidacy> candidacies, boolean file, String documentNumber, boolean pos){
		...
		String FILE;
		if (pos)
			FILE = "nonAdmitedByPos"+documentNumber+".pdf";
		else
			FILE = "nonAdmited"+documentNumber+".pdf";
		try {
			...
			//add text
			addTitlePage(document, pos);
			Paragraph preface = new Paragraph();
			addEmptyLine(preface, 2);
			document.add(preface);
			// add chart

			Image image2 = Image.getInstance("nonAdm.png");
			document.add(image2);
			...
			PdfPCell cell2 = new PdfPCell(new Paragraph("Motivo"));
			...
			for (ICandidacy interv:candidacies){
				...
				table.addCell(interv.getRejectionReason().toString());
			}
			document.add(table);
	...
	private void addTitlePage(Document document,boolean pos)
			throws DocumentException {

		Paragraph preface = new Paragraph();
		addEmptyLine(preface, 1);
		if (pos)
			preface.add(new Paragraph("Relatório Candidatos Rejeitados Por Posição Período De Tempo", catFont));
		else
			preface.add(new Paragraph("Relatório Candidatos Rejeitados Por Período De Tempo", catFont));
