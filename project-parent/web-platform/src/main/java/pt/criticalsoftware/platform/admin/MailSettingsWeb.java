...
public class MailSettingsWeb {
	...
	public void newSettings() {
		try {
		IEmail newSettings = builder.hostname(hostname).smtpPort(smtpPort)
				.username(username).password(password).sslOnConnect(sslOnConnect).startTLS(startTLS)
				.active(active).build();
		business.addSettings(newSettings);
		FacesMessage msg = new FacesMessage(FacesMessage.SEVERITY_INFO,"Novas definições guardadas com sucesso.",null);
